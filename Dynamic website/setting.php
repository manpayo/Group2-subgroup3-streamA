...
if (isset($_POST['save'])) {
    ...
    // Validate input
    $errors = array();
    if(empty($Username) || empty($Bio) || empty($Gender) || empty($Relationship)){
    ...
    if(count($errors) > 0){
        ...
        $sql = "UPDATE register SET Username=?, Bio=?, Gender=?, Relationship=? WHERE UserID=?";
        $stmt = mysqli_stmt_init($CONNECT);
        $prepareStmt = mysqli_stmt_prepare($stmt, $sql);
        if ($prepareStmt) {
            mysqli_stmt_bind_param($stmt, "ssssi", $Username, $Bio, $Gender, $Relationship, $UserID);
                        ...
                        <div>
                            ...
                            <div class="max-w-md mx-auto">
                                ...
                                <div class="flex items-center justify-center gap-4 mt-16">
                                    <button type="submit" class="button lg:px-6 bg-secondery max-md:flex-1"> Cancle</button>
                                    <button type="submit" class="button lg:px-10 bg-primary text-white max-md:flex-1" name="save"> Save</button>
                                </div>
